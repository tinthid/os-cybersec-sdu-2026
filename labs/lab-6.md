...
2. สร้าง thread ด้วย **Pthreads** และสังเกตด้วย `ps`, `htop`, `/proc`
3. เห็นว่า thread **share memory** กัน (ต่างจาก process ที่แยกกัน)
4. เข้าใจ Threading Issues — พฤติกรรมของ **fork()**, **signal** และ **clone()** ใน multithreaded program
5. เข้าใจความแตกต่างระหว่าง **Concurrency** กับ **Parallelism**
...
## Part 2 — สร้าง Thread ด้วย Pthreads และสังเกต

### 2.1 สร้าง thread แล้วสังเกตด้วย ps และ /proc
...
> **คำถาม 2.1:** จากผลลัพธ์ของ `ps -T` thread ทั้ง 4 ตัวมี PID เหมือนกันหรือไม่? SPID ต่างกันหรือไม่? สิ่งนี้บอกอะไรเกี่ยวกับ thread?
>
> ```
> ตอบ:
>
...
> **คำถาม 2.2:** ใน `/proc/<PID>/task/` แต่ละ thread มีฟิลด์ `Tgid` (Thread Group ID) เหมือนกันหรือไม่? `Tgid` คืออะไร?
>
> ```
> ตอบ:
>
...
> **กฎ compile:** ต้องใส่ `-lpthread` ทุกครั้ง!
> ```bash
...
## Part 3 — Thread Share Memory กัน (พิสูจน์!)

### 3.1 ทบทวน: fork() แยก memory
...
> **คำถาม 3.1:** ถ้าใช้ `fork()` แทน `pthread_create()` ค่า counter ของ parent จะเป็นเท่าไหร่หลัง child เพิ่มเป็น 5? ทำไม?
>
> ```
> ตอบ:
>
...
### 3.3 ดู memory map เปรียบเทียบ

ลองสังเกตว่า thread ทุกตัว share address space เดียวกัน (ใช้ `thread_observe` จาก Part 2):
...
> **คำถาม 3.2:** memory map ของ thread ต่าง ๆ ในโปรแกรมเดียวกันเหมือนหรือต่างกัน? ทำไม?
>
> ```
> ตอบ:
>
...
## Part 4 — Threading Issues

### 4.1 fork() ใน multithreaded program
...
ทำไม Linux ไม่ copy ทุก thread ไปด้วย?
- thread อื่นอาจกำลัง **ถือ lock** อยู่ → child จะ **deadlock** เพราะไม่มีใคร unlock
- thread อื่นอาจกำลังทำ **I/O ค้าง** → copy ไปจะเกิดปัญหา
- **ซับซ้อนและช้ามาก** → ต้อง copy stack + synchronize สถานะของทุก thread

> **กฎ:** `fork()` copy แค่ 1 thread, `exec()` ลบทุก thread แล้วเริ่มใหม่

> **คำถาม 4.1:** ถ้า multithreaded web server ต้อง fork+exec เพื่อรัน CGI script จะเกิดอะไรกับ worker threads อื่น ๆ ใน child?
>
> ```
> ตอบ:
>
...
### 4.2 Signal ใน multithreaded program
...
ลองส่ง signal ไปที่ `thread_observe` ที่กำลังรันอยู่:

```bash
./thread_observe &

# ส่ง SIGUSR1 ไป — ดูว่าเกิดอะไร
kill -USR1 $(pgrep thread_observe)
```

> **คำถาม 4.2:** ถ้าต้องการให้ thread เฉพาะตัวเดียวรับ signal ทุกครั้ง ต้องทำอย่างไร?
>
> ```
> ตอบ:
>
...
### 4.3 Linux Threads ภายใน — clone() System Call

Linux ไม่ได้แยก "process" กับ "thread" จริง ๆ — ภายใน kernel ทุกอย่างเป็น **task** ที่สร้างด้วย `clone()` system call ตัวเดียวกัน สิ่งที่ต่างคือ **flags**:

| Flag | ความหมาย | fork() | pthread_create() |
|---|---|---|---|
| `CLONE_VM` | share memory space | ไม่ใช้ (copy memory) | **ใช้** |
| `CLONE_FILES` | share open files | ไม่ใช้ (copy) | **ใช้** |
...
ลอง strace ดู clone() flags จริง:
...
> **สังเกต:** จะเห็น `clone3()` พร้อม flags `CLONE_VM|CLONE_FS|CLONE_FILES|CLONE_SIGHAND|CLONE_THREAD` — share ทุกอย่าง!

> **คำถาม 4.3:** flag ไหนที่ทำให้ thread share memory กัน? ถ้าไม่มี flag นี้จะเกิดอะไรขึ้น?
>
> ```
> ตอบ:
>
...
```
  Concurrency (1 core):     ┃T1┃T2┃T3┃T1┃T2┃T3┃T1┃...
                             └──── สลับกัน ────┘

  Parallelism (2 cores):    core 1: ┃T1┃T3┃T1┃T3┃...
                             core 2: ┃T2┃T4┃T2┃T4┃...
                             └── ทำพร้อมกันจริง ──┘
```

```bash
# ดูจำนวน CPU cores ของเครื่อง
nproc
```

> **คำถาม 4.4:** อธิบายความแตกต่างระหว่าง concurrency กับ parallelism ด้วยคำพูดของตัวเอง ถ้าเครื่องมี 1 core โปรแกรม multithreaded ยังทำงานได้หรือไม่?
>
> ```
> ตอบ:
>
...
## Part 5 — Thread vs Process: สรุปเปรียบเทียบ
...
| งานที่สร้าง/ทำลายบ่อย | **เหมาะ** (เร็ว เบา) | ไม่เหมาะ (ช้า หนัก) |
...
> **คำถาม 5.1:** ถึงแม้ thread จะเบากว่า แต่มีกรณีไหนบ้างที่ควรใช้ process (fork) แทน? (ยกตัวอย่างอย่างน้อย 2 กรณี)
>
> ```
> ตอบ:
>
...
> **คำถาม 5.2:** ทำไม Chrome ถึงเลือกใช้ process แยกแต่ละ tab แทนที่จะใช้ thread?
>
> ```
> ตอบ:
>
...
| `strace -f -e trace=clone3,clone` | ดู clone() system call |
...
3. สร้าง thread ด้วย **`pthread_create()`** และรอด้วย **`pthread_join()`**
4. Thread **share memory** กัน — ต่างจาก fork() ที่แยก memory คนละชุด
5. **Threading Issues:** `fork()` ใน multithreaded program จะ copy แค่ thread ที่เรียก, signal อาจถูกรับโดย thread ใดก็ได้
6. **Linux ใช้ `clone()` system call** สร้างทั้ง process และ thread — ต่างกันที่ **flags** (CLONE_VM, CLONE_THREAD ฯลฯ)
7. **Concurrency** คือหลาย task สลับทำงาน, **Parallelism** คือทำงานพร้อมกันจริงบนหลาย cores
